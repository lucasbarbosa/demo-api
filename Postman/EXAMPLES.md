...
> **💡 Note:** The examples below use `{{baseUrl}}` which will be replaced by the actual URL based on your selected environment:
> - **DemoApi - Local:** `http://localhost:5100/api/v1`
> - **DemoApi - Docker:** `http://localhost:5200/api/v1`
> - **DemoApi - Swagger:** `http://localhost:5084/api/v1`
...
**Example URLs:**
- Local: `http://localhost:5100/api/v1/auth/token`
- Docker: `http://localhost:5200/api/v1/auth/token`
...
Content-Type: application/json
Authorization: Bearer {token}  # Only for JWT collection
```

**Example URLs:**
- Swagger: `http://localhost:5084/api/v1/products`
- Local: `http://localhost:5100/api/v1/products`
- Docker: `http://localhost:5200/api/v1/products`
...
## 🔒 Authentication Errors (JWT Collection)
...
- The post-request script automatically saves the token in the `{{token}}` environment variable
...
### Environments
- **DemoApi - Local**: Use for local development on port 5100
- **DemoApi - Docker**: Use for Docker testing on port 5200
